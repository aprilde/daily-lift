...
**Status: complete.** The plan is locked, the native build is done, and the app installs and runs. The app's source code lives at the repo root (`../app/`, standard Gradle project layout), not in this folder - this folder holds the plan, the process record, and the review of the finished result.

## What's here

- `stage3-build-prompt-DRAFT.md` - the raw build plan, before team review hardened it.
- `BUILD-DIARY.md` - a running diary of the build session.
- `test-plan.md` - the full v1 test plan, every case derived from the hardened build plan and the decision log.
- `design-review.md` - a post-build review of the finished screens against the original design intent.
- `design-review-prototype.html` - the reference prototype used during that review.
- `screenshots/` - screens captured from the running app during the design review.
